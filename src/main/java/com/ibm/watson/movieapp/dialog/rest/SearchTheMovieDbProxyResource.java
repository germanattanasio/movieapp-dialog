...
/**
 * Proxy class to communicate with TMDB
 * @author Ashima Arora
 * This class acts as a proxy on the server-side to communicate with themoviedb.org via RESTful API calls and relays back movie information to the client-side.
 * <p>
 * There are two entry points into this class via JAX-RS calls/internal calls made by {@code WDSBlueMixProxyResource} to gather movie data.
 * eg.: /watson-movieapp-dialog/api/movies/getMovieDetails is one such entry point.
 * <p>
...
public class SearchTheMovieDbProxyResource {
	...
	private static LoadingCache<String, String> loadTheMovieDbCache(){ 
	    return CacheBuilder.newBuilder().initialCapacity(8)
	            .expireAfterWrite(1, TimeUnit.DAYS) //refresh once a day
	            .maximumSize(20)
	            .concurrencyLevel(5)
	 ...
	 * Hashtable elements are of the form {uri_parameter_name, uri_parameter_value}
	 ...
	 */
	public WDSConversationPayload discoverMovies(String genre, String rating, String recency, int currentIndex, int pageNum, boolean searchForward) throws ClientProtocolException, IllegalStateException, IOException, HttpException, URISyntaxException, WatsonTheatersException, ParseException {
	 ...
	 */
	public Integer getGenreId(String userGenre) throws URISyntaxException, ClientProtocolException, IllegalStateException, IOException, HttpException, WatsonTheatersException {

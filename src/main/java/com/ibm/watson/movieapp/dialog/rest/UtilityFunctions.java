...
public final class UtilityFunctions {
	...
	static{
		String loggingLevel = System.getProperty("watson.theaters.logger.level"); //$NON-NLS-1$
		if(loggingLevel != null){
			loggingLevel = loggingLevel.toUpperCase();
			LoggerContext ctx = (LoggerContext) LogManager.getContext(false);
			Configuration config = ctx.getConfiguration();
			LoggerConfig loggerConfig = config.getLoggerConfig(LogManager.ROOT_LOGGER_NAME); 
			Level level = Level.getLevel(loggingLevel);
			loggerConfig.setLevel(level);
			loggerConfig = config.getLoggerConfig(logger.getName()); 
			loggerConfig.setLevel(level);
			loggerConfig = config.getLoggerConfig("org.apache.wink"); //$NON-NLS-1$
	 ...
	 * <p>
	 * This extracts a JSON object response from a CloseableHttpResponse.
	 ...
	 * <p>
	 * This makes HTTP PUT requests to the url provided.
	 ...
	 * @throws HttpException if the HTTP call responded with a status code other than 200 or 201
	 */
	public static JsonObject httpPut(CloseableHttpClient httpClient, String url, JsonObject requestJson) throws ClientProtocolException, IOException, IllegalStateException, HttpException{
	 ...
	 * @throws HttpException if the HTTP call responded with a status code other than 200 or 201
	 */
	public static JsonObject httpPost(CloseableHttpClient httpClient, String url, List<NameValuePair> nvps) throws ConnectException, ClientProtocolException, IOException, IllegalStateException, HttpException{
	 ...
	 * <p>
	 * This returns a property's value from a JSON object.

...
public class SearchTheMovieDbProxyResourceTest {
	...
	public void testinvalidgetMovieDetails() throws Exception {
		...
		when(moviedbresource.getMovieDetails(null, "The Hunger Games:Mockingjay Part 1"))
		.thenReturn(response);
		...
		verify(moviedbresource, times(1)).getMovieDetails(argumentCaptor.capture(),
				Mockito.eq("The Hunger Games:Mockingjay Part 1"));
	...
	public void testvalidgetMovieDetails() throws Exception {
		...
		when(
				moviedbresource.getMovieDetails("131631",
						"The Hunger Games:Mockingjay Part 1")).thenReturn(
								response);
		ArgumentCaptor<String> argumentCaptor = ArgumentCaptor
				.forClass(String.class);
		moviedbresource.getMovieDetails("131631", "The Hunger Games:Mockingjay Part 1");
		verify(moviedbresource, times(1)).getMovieDetails(Mockito.eq("131631"),
				argumentCaptor.capture());
		assertEquals("The Hunger Games:Mockingjay Part 1",
				argumentCaptor.getValue());
		assertEquals(response.getStatus(), 200);
		assertNotNull(response.getEntity());
	}
	/**
	 * Tests the discoverMovies method with a mock WDSConversationPayload.
	 * Return WatsonTheatersException if recency is invalid
	 */
	...
	public void testdiscoverMovies() throws Exception {
		...
		when(moviedbresource.discoverMovies("action", "R", "Upcoming", 1, 1, false))
		.thenReturn(conversationPayload);
		...
		when(moviedbresource.discoverMovies("action", "R", null, 1, 1, false))
		.thenThrow( new WatsonTheatersException(errorMessage, issue));
		WDSConversationPayload response_payload = moviedbresource.discoverMovies("action",
				"R", "Upcoming", 1, 1, false);
		assertEquals(response.getStatus(), 200);
	...
	public void testgetResults() throws Exception {
		String jArrayString = "[{\"id\":321494,\"title\":\"The Man with the Iron Fists 2\",\"vote_average\":5.3,\"backdrop_path\":\"/nqL7K2iWoMGrDWRqq6Jgn1s56d6.jpg\",\"release_date\":\"2015-04-14\",\"original_title\":\"The Man with the Iron Fists 2\",\"vote_count\":3,\"adult\":false,\"poster_path\":\"/njPa0RO0KKmwBCGHcEOTbxljftF.jpg\",\"video\":false,\"popularity\":5.04235888862094},{\"id\":331963,\"title\":\"Hooligans at War: North vs South\",\"vote_average\":10.0,\"backdrop_path\":\"/m8A1yhr9sqbItYhxLZk6lnUhoQF.jpg\",\"release_date\":\"2015-04-13\",\"original_title\":\"Hooligans at War: North vs South\",\"vote_count\":1,\"adult\":false,\"poster_path\":\"/sOoTQhKty7SE3sF8yU0BfEnACVG.jpg\",\"video\":false,\"popularity\":2.2753683900707}]";
	...
	public void testaddDateFilters() throws Exception {
		...
		assertEquals(dateFormat.format(date),
				uriParamsHash.get("primary_release_date.gte"));
		uriParamsHash = movieDbProxy.addDateFilters("current", uriParamsHash);
		assertNotNull(uriParamsHash);
		assertEquals(dateFormat.format(date),
				uriParamsHash.get("primary_release_date.lte"));
	}
	/**
	 ...
	 */
	protected SearchTheMovieDbProxyResource mock_search_movie_rest_client_is_created(
			int status_code, Object entity) throws Exception {
	 ...
	 */
	protected WDSBlueMixProxyResource mock_bluemix_rest_client_is_created(
			int status_code, Object entity) throws Exception {
